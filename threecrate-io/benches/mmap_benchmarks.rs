...
use threecrate_io::{PcdDataFormat, PcdWriteOptions, RobustPcdReader, RobustPcdWriter};
use threecrate_io::{PlyWriteOptions, RobustPlyReader, RobustPlyWriter};
...
fn generate_test_point_cloud(num_points: usize) -> PointCloud<Point3f> {
    let points: Vec<Point3f> = (0..num_points)
        .map(|i| {
            let i = i as f32;
            Point3f::new((i * 0.1).sin(), (i * 0.1).cos(), i * 0.001)
        })
        .collect();
...
fn benchmark_ply_reading(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        #[cfg(feature = "io-mmap")]
        {
            group.bench_with_input(BenchmarkId::new("mmap_enabled", size), &size, |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("buffered_io", size), &size, |b, _| {
            b.iter(|| {
                let file = File::open(temp_file.path()).expect("Failed to open file");
                let mut reader = std::io::BufReader::new(file);
                let ply_data =
                    RobustPlyReader::read_ply_data(&mut reader).expect("Failed to read PLY file");
                black_box(ply_data);
            });
        });
    }
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("buffered_io", size), &size, |b, _| {
            b.iter(|| {
                ...
                let mut reader = std::io::BufReader::new(file);
                let (header, points) =
                    RobustPcdReader::read_pcd_data(&mut reader).expect("Failed to read PCD file");
                black_box((header, points));
            });
        });
    }
...
fn benchmark_small_file_overhead(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        #[cfg(feature = "io-mmap")]
        {
            group.bench_with_input(BenchmarkId::new("mmap_small", size), &size, |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("buffered_small", size), &size, |b, _| {
            b.iter(|| {
                let file = File::open(temp_file.path()).expect("Failed to open file");
                let mut reader = std::io::BufReader::new(file);
                let ply_data =
