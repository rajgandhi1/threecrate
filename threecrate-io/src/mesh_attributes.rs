...
use threecrate_core::{Error, Result, TriangleMesh, Vector3f};
...
impl ExtendedTriangleMesh {
    ...
    pub fn validate_attributes(&mut self) -> Result<()> {
        ...
        if let Some(ref normals) = self.mesh.normals {
            if normals.len() != vertex_count {
                let msg = format!(
                    "Normal count mismatch: {} normals for {} vertices",
                    normals.len(),
                    vertex_count
                );
        ...
        if let Some(ref uvs) = self.uvs {
            if uvs.len() != vertex_count {
                let msg = format!(
                    "UV count mismatch: {} UVs for {} vertices",
                    uvs.len(),
                    vertex_count
                );
                self.metadata.validation_messages.push(msg.clone());
            ...
            for (i, uv) in uvs.iter().enumerate() {
                if !uv[0].is_finite() || !uv[1].is_finite() {
                    let msg = format!(
                        "Invalid UV coordinates at vertex {}: [{}, {}]",
                        i, uv[0], uv[1]
                    );
                    self.metadata.validation_messages.push(msg);
                }
        ...
        if let Some(ref tangents) = self.tangents {
            if tangents.len() != vertex_count {
                let msg = format!(
                    "Tangent count mismatch: {} tangents for {} vertices",
                    tangents.len(),
                    vertex_count
                );
                self.metadata.validation_messages.push(msg.clone());
            ...
            for (i, tangent) in tangents.iter().enumerate() {
                ...
                if tangent.handedness.abs() != 1.0 {
                    let msg = format!(
                        "Invalid tangent handedness at vertex {}: {}",
    ...
    pub fn compute_normals(&mut self, smooth: bool, normalize: bool) -> Result<()> {
        ...
        if normalize {
            for normal in &mut normals {
                ...
                if length > 1e-6 {
                    *normal =
                        Vector3f::new(normal.x / length, normal.y / length, normal.z / length);
                } else {
    ...
    pub fn compute_tangents(&mut self, normalize: bool) -> Result<()> {
        let uvs = self.uvs.as_ref().ok_or_else(|| {
            Error::InvalidData("UV coordinates required for tangent computation".to_string())
        })?;
        ...
        let normals = self.mesh.normals.as_ref().ok_or_else(|| {
            Error::InvalidData("Normals required for tangent computation".to_string())
        })?;
        ...
        for i in 0..vertex_count {
            ...
            let handedness =
                if cross.x * tan2[i].x + cross.y * tan2[i].y + cross.z * tan2[i].z < 0.0 {
                    -1.0
                } else {
                    1.0
                };
    ...
    pub fn generate_default_uvs(&mut self) -> Result<()> {
        ...
        if size_x >= size_y && size_x >= size_z {
            ...
            for vertex in &self.mesh.vertices {
                let u = if size_y > 1e-6 {
                    (vertex.y - min_y) / size_y
                } else {
                    0.5
                };
                let v = if size_z > 1e-6 {
...
pub mod utils {
    ...
    pub fn prepare_for_serialization(mesh: &mut ExtendedTriangleMesh, format: &str) -> Result<()> {
