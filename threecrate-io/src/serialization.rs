...
impl AttributePreservingWriter {
    ...
    fn write_ply_extended<P: AsRef<Path>>(
        ...
        if let Some(ref uvs) = mesh.uvs {
            let u_values: Vec<ply::PlyValue> =
                uvs.iter().map(|uv| ply::PlyValue::Float(uv[0])).collect();
            let v_values: Vec<ply::PlyValue> =
                uvs.iter().map(|uv| ply::PlyValue::Float(uv[1])).collect();
        ...
        if let Some(ref tangents) = mesh.tangents {
            let tx_values: Vec<ply::PlyValue> = tangents
                .iter()
                .map(|t| ply::PlyValue::Float(t.vector.x))
                .collect();
            let ty_values: Vec<ply::PlyValue> = tangents
                .iter()
                .map(|t| ply::PlyValue::Float(t.vector.y))
                .collect();
            let tz_values: Vec<ply::PlyValue> = tangents
                .iter()
                .map(|t| ply::PlyValue::Float(t.vector.z))
                .collect();
            let th_values: Vec<ply::PlyValue> = tangents
                .iter()
                .map(|t| ply::PlyValue::Float(t.handedness))
                .collect();
        ...
        for (name, values) in &options.custom_properties {
            let ply_values: Vec<ply::PlyValue> =
                values.iter().map(|&v| ply::PlyValue::Float(v)).collect();
        ...
        for &face_indices in &mesh.mesh.faces {
            ...
            for &vertex_idx in &face_indices {
                let face_vertex = obj::FaceVertex {
                    vertex: vertex_idx,
                    texture: if mesh.uvs.is_some() {
                        Some(vertex_idx)
                    } else {
                        None
                    },
                    normal: if mesh.mesh.normals.is_some() {
                        Some(vertex_idx)
                    } else {
                        None
                    },
...
pub mod utils {
    ...
    pub fn prepare_mesh_for_format(mesh: &mut ExtendedTriangleMesh, format: &str) -> Result<()> {
...
mod tests {
    ...
    fn test_mesh_comparison() {
        ...
        assert!(
            differences.is_empty(),
            "Identical meshes should have no differences"
        );
    }
}
