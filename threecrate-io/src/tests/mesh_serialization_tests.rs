...
use crate::{
    ...
    serialization::{
        utils, AttributePreservingReader, AttributePreservingWriter, SerializationOptions,
    },
};
use std::fs;
...
mod attribute_tests {
    ...
    fn test_normal_computation() {
        ...
        for normal in &normals {
            assert!(
                (normal.z - 1.0).abs() < 1e-5,
                "Normal should point up: {:?}",
                normal
            );
            assert!(
                (normal.x * normal.x + normal.y * normal.y + normal.z * normal.z - 1.0).abs()
                    < 1e-5,
...
mod serialization_tests {
    ...
    fn test_obj_round_trip_basic() {
        ...
        // Write mesh
        AttributePreservingWriter::write_extended_mesh(&original_mesh, temp_file, &options)
            .unwrap();

        // Read mesh back
        let loaded_mesh =
            AttributePreservingReader::read_extended_mesh(temp_file, &options).unwrap();
    ...
    fn test_cross_format_conversion() {
        ...
        let obj_mesh =
            AttributePreservingReader::read_extended_mesh(obj_file, &serialization_options)
                .unwrap();
        AttributePreservingWriter::write_extended_mesh(&obj_mesh, ply_file, &serialization_options)
            .unwrap();

        // Read PLY back
        let ply_mesh =
            AttributePreservingReader::read_extended_mesh(ply_file, &serialization_options)
                .unwrap();
    ...
    fn test_validation_on_read() {
        ...
        AttributePreservingWriter::write_extended_mesh(&original_mesh, temp_file, &write_options)
            .unwrap();
        ...
        let loaded_mesh =
            AttributePreservingReader::read_extended_mesh(temp_file, &read_options).unwrap();
...
mod integration_tests {
    ...
    fn test_round_trip_utility() {
        ...
        // Write initial mesh
        let options = SerializationOptions::preserve_all();
        AttributePreservingWriter::write_extended_mesh(&original_mesh, input_file, &options)
            .unwrap();

        // Test round-trip utility
        let (final_mesh, warnings) =
            utils::test_round_trip(input_file, output_file, Some(options)).unwrap();
    ...
    fn test_custom_properties() {
        ...
        assert!(
            content.contains("intensity"),
            "Custom property not found in PLY file"
        );
...
mod edge_case_tests {
    ...
    fn test_large_mesh_handling() {
        ...
        .unwrap();
        let loaded_mesh =
            AttributePreservingReader::read_extended_mesh(temp_file, &serialization_options)
                .unwrap();
