...
impl AlphaComplex {
    ...
    fn generate_triangles_fast(&mut self) -> Result<()> {
        let max_triangles = if self.points.len() < 1000 {
            50000
        } else {
            100000
        };
        ...
        for i in 0..self.points.len() {
            ...
            for (idx_j, &j) in neighbors.iter().enumerate() {
                ...
                for &k in neighbors.iter().skip(idx_j + 1) {
                    ...
                    if triangle
                        .is_alpha_valid(self.config.alpha * 1.5, self.config.min_triangle_area)
                    {
    ...
    fn generate_triangles_quality(&mut self) -> Result<()> {
        let mut edge_triangles: std::collections::HashMap<(usize, usize), Vec<usize>> =
    ...
    fn filter_simplices(&mut self) -> Result<()> {
        match self.config.mode {
            AlphaMode::BoundaryOnly => {
                self.simplices.retain(|s| {
                    s.is_boundary
                        && s.is_alpha_valid(self.config.alpha, self.config.min_triangle_area)
                });
            }
            AlphaMode::Full => {
                self.simplices
                    .retain(|s| s.is_alpha_valid(self.config.alpha, self.config.min_triangle_area));
            }
            AlphaMode::Adaptive => {
    ...
    pub fn extract_surface(&mut self) -> Result<TriangleMesh> {
        ...
        if self.simplices.is_empty() {
            return Err(Error::Algorithm(
                "No candidate triangles generated".to_string(),
            ));
        }
        ...
        if self.simplices.is_empty() {
            return Err(Error::Algorithm(
                "No triangles survived alpha filtering".to_string(),
            ));
        }
    ...
    pub fn get_statistics(&self) -> AlphaComplexStats {
        ...
        let avg_circumradius = if !self.simplices.is_empty() {
            self.simplices
                .iter()
                .map(|s| s.circumradius_sq.sqrt())
                .sum::<f32>()
                / self.simplices.len() as f32
        } else {
            0.0
        };
...
pub fn alpha_complex_reconstruction(
...
pub fn estimate_optimal_alpha(cloud: &PointCloud<Point3f>, k: usize) -> Result<f32> {
    ...
    for i in (0..cloud.points.len()).step_by(step) {
        ...
        let mut point_distances: Vec<f32> = cloud
            .points
            .iter()
            .enumerate()
            .filter(|(j, _)| *j != i)
