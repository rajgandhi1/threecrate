...
pub fn segment_plane_parallel(
    ...
    let best = results.into_iter().max_by_key(|(_, _, score)| *score);
...
mod tests {
    ...
    fn test_plane_distance_calculation() {
        ...
        assert_relative_eq!(
            model.distance_to_point(&point_on_plane),
            0.0,
            epsilon = 1e-6
        );
        assert_relative_eq!(
            model.distance_to_point(&point_above_plane),
            1.0,
            epsilon = 1e-6
        );
        assert_relative_eq!(
            model.distance_to_point(&point_below_plane),
            1.0,
            epsilon = 1e-6
        );
    }
    ...
    fn test_segment_plane_ransac_noisy() {
        ...
        assert!(
            inliers.len() >= 350,
            "Should find most planar points as inliers"
        );
        ...
        let outlier_inliers: Vec<usize> = inliers
            .iter()
            .filter(|&&idx| outlier_indices.contains(&idx))
            .cloned()
            .collect();
        assert!(
            outlier_inliers.len() <= 2,
            "Should not include many outliers in inliers"
        );
    }
    ...
    fn test_segment_plane_ransac_tilted_plane() {
        ...
        assert!(
            dot_product > 0.8,
            "Normal should be close to expected direction: {:?}",
            normal
        );
    }
    ...
    fn test_plane_segmentation_ransac_alias() {
        ...
        assert!(
            diff <= 5,
            "Inlier counts should be similar: {} vs {}",
            result1.1.len(),
            result2.1.len()
        );
    }
    ...
    fn test_euclidean_cluster_three_blobs() {
        let mut cloud = PointCloud::new();
        for p in make_sphere_cloud(Point3f::new(0.0, 0.0, 0.0), 0.4, 300) {
            cloud.push(p);
        }
        for p in make_sphere_cloud(Point3f::new(5.0, 0.0, 0.0), 0.4, 200) {
            cloud.push(p);
        }
        for p in make_sphere_cloud(Point3f::new(0.0, 5.0, 0.0), 0.4, 100) {
            cloud.push(p);
        }
    ...
    fn test_euclidean_cluster_min_size_filter() {
        ...
        for p in make_sphere_cloud(Point3f::new(10.0, 0.0, 0.0), 0.2, 5) {
            cloud.push(p);
        }
    ...
    fn test_euclidean_cluster_max_size_filter() {
        let mut cloud = PointCloud::new();
        for p in make_sphere_cloud(Point3f::new(0.0, 0.0, 0.0), 0.5, 500) {
            cloud.push(p);
        }
        ...
        assert_eq!(
            result.num_clusters(),
            0,
            "Oversized cluster should be filtered"
        );
    }
    ...
    fn test_euclidean_cluster_get_cloud() {
        ...
        for p in make_sphere_cloud(Point3f::new(10.0, 0.0, 0.0), 0.3, 100) {
            cloud.push(p);
        }
    ...
    fn test_euclidean_cluster_parallel_matches_sequential() {
        ...
        for p in make_sphere_cloud(Point3f::new(5.0, 0.0, 0.0), 0.3, 150) {
            cloud.push(p);
        }
    ...
    fn test_euclidean_cluster_invalid_config() {
        ...
        assert!(
            extract_euclidean_clusters(&cloud, &EuclideanClusterConfig::new(-1.0, 1, 100)).is_err()
        );
        assert!(
            extract_euclidean_clusters(&cloud, &EuclideanClusterConfig::new(0.1, 0, 100)).is_err()
        );
        assert!(
            extract_euclidean_clusters(&cloud, &EuclideanClusterConfig::new(0.1, 10, 5)).is_err()
        );
    }
