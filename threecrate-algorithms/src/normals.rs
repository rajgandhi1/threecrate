...
fn orient_normal_towards_viewpoint(
    normal: Vector3f,
    point: Point3f,
    viewpoint: Point3f,
) -> Vector3f {
...
pub fn estimate_normals_inplace(_cloud: &mut PointCloud<Point3f>, k: usize) -> Result<()> {
    ...
    Err(Error::Unsupported(
        "Use estimate_normals instead".to_string(),
    ))
}
...
mod tests {
    ...
    fn test_estimate_normals_radius() {
        ...
        assert!(
            percentage > 80.0,
            "Only {:.1}% of normals are in Z direction",
            percentage
        );
    }
    ...
    fn test_estimate_normals_cylinder() {
        ...
        println!(
            "Cylinder test: {:.1}% perpendicular to Z, {:.1}% pointing outward",
            percentage_perpendicular, percentage_outward
        );

        // For a cylinder, normals should be perpendicular to Z-axis
        assert!(
            percentage_perpendicular > 60.0,
            "Only {:.1}% of normals are perpendicular to Z-axis",
            percentage_perpendicular
        );
    }
    ...
    fn test_estimate_normals_orientation_consistency() {
        ...
        let _result_inconsistent =
            estimate_normals_with_config(&cloud, &config_inconsistent).unwrap();
        ...
        for point in result_consistent.iter() {
            assert!(
                (point.normal.z * first_normal_consistent) > 0.0,
