...
fn write_complex_obj_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let texture_coords = vec![
        ...
        [1.0, 0.0],
        [1.0, 1.0],
        [0.0, 1.0], // Basic quad UVs
        [0.0, 0.0],
        [1.0, 0.0],
        [1.0, 1.0],
        [0.0, 1.0], // Repeat for other faces
    ];
...
fn test_round_trip() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (orig, loaded) in original_mesh
        .vertices
        .iter()
        .zip(loaded_mesh.vertices.iter())
    {
        let error = ((orig.x - loaded.x).powi(2)
            + (orig.y - loaded.y).powi(2)
            + (orig.z - loaded.z).powi(2))
        .sqrt();
    ...
    if let (Some(orig_normals), Some(loaded_normals)) =
        (&original_mesh.normals, &loaded_mesh.normals)
    {
...
fn write_mtl_file() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   ✓ Created example_materials.mtl with {} materials",
        materials.len()
    );
    ...
    println!(
        "   ✓ Successfully read back {} materials",
