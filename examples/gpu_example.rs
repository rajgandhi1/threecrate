...
use threecrate_gpu::{gpu_estimate_normals, gpu_icp, gpu_remove_statistical_outliers, GpuContext};
...
async fn main() -> threecrate_core::Result<()> {
    ...
    let start = Instant::now();
    let filtered_cloud =
        gpu_remove_statistical_outliers(&gpu_context, &target_cloud, 10, 2.0).await?;
    ...
    println!(
        "   - Removed: {} outliers",
        target_cloud.len() - filtered_cloud.len()
    );
    ...
    println!("\n⚡ Direct GPU context usage:");
    let start = Instant::now();
    let normals_vec = gpu_context
        .compute_normals(&source_cloud.points, 10)
        .await?;
