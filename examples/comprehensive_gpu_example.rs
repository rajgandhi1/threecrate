...
use threecrate_gpu::{
    colored_point_cloud_to_vertices, create_tsdf_volume, gpu_estimate_normals, gpu_icp,
    gpu_remove_statistical_outliers, gpu_tsdf_extract_surface, gpu_tsdf_integrate,
    mesh_to_gpu_mesh, point_cloud_to_vertices_colored, CameraIntrinsics, GpuContext, GpuMesh,
    MeshLightingParams, MeshRenderConfig, PbrMaterial, RenderConfig, RenderParams,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✓ Generated sample point clouds ({} and {} points)",
        source_cloud.points.len(),
        target_cloud.points.len()
    );
    ...
    println!(
        "✓ Estimated {} normals in {:.2}ms",
        normals_result.points.len(),
        normals_time.as_millis()
    );
    ...
    let start = Instant::now();
    let filtered_cloud =
        gpu_remove_statistical_outliers(&gpu_context, &source_cloud, 10, 1.0).await?;
    let filter_time = start.elapsed();
    println!(
        "✓ Filtered {} -> {} points in {:.2}ms",
        source_cloud.points.len(),
        filtered_cloud.points.len(),
    ...
    println!(
        "✓ ICP registration completed in {:.2}ms",
        icp_time.as_millis()
    );
    ...
    println!(
        "✓ TSDF integration completed in {:.2}ms",
        tsdf_time.as_millis()
    );
    println!(
        "  Volume: {}x{}x{} voxels",
    ...
    println!(
        "✓ Surface extraction completed in {:.2}ms",
    ...
    let point_positions: Vec<Point3<f32>> = filtered_cloud
        .points
        .iter()
    ...
    println!(
        "✓ Mesh setup completed in {:.2}ms",
        mesh_setup_time.as_millis()
    );
    println!(
        "  Created cube mesh: {} vertices, {} indices",
        cube_mesh.vertices.len(),
        cube_mesh.indices.len()
    );
    println!(
        "  Created triangle mesh: {} vertices, {} indices",
        triangle_mesh.vertices.len(),
        triangle_mesh.indices.len()
    );
    println!(
        "  Created point cloud mesh: {} vertices, {} indices",
        point_cloud_mesh.vertices.len(),
        point_cloud_mesh.indices.len()
    );
    ...
    let filtered_vertices =
        point_cloud_to_vertices_colored(&filtered_cloud, render_config.render_params.point_size);
    let surface_vertices =
        colored_point_cloud_to_vertices(&surface_cloud, render_config.render_params.point_size);

    println!(
        "✓ Converted {} filtered points to render vertices",
        filtered_vertices.len()
    );
    println!(
        "✓ Converted {} surface points to render vertices",
    ...
    println!(
        ...
        mesh_config.lighting_params.light_intensity
    );
    println!(
        "  • Gamma correction: {}",
        mesh_config.lighting_params.gamma
    );
    println!(
        "  • MSAA: {}x",
        if mesh_config.enable_multisampling {
            "4"
        } else {
            "1"
        }
    ...
    println!(
        "  • MSAA: {}x (anti-aliasing)",
        if render_config.enable_multisampling {
            "4"
        } else {
            "1"
        }
    ...
    println!(
        "  • Point Size: {:.1}px",
        render_config.render_params.point_size
    );
    println!(
        "  • Alpha Threshold: {:.2}",
        render_config.render_params.alpha_threshold
    );
    ...
    println!(
        "  • Lighting: Ambient({:.1}) + Diffuse({:.1}) + Specular({:.1})",
    ...
    println!("---------------------");
    let total_time =
        normals_time + filter_time + icp_time + tsdf_time + extraction_time + mesh_setup_time;
    ...
    println!(
        "  • Cube: Albedo({:.1}, {:.1}, {:.1}), Metallic: {:.1}, Roughness: {:.1}",
        cube_mesh.material.albedo[0],
        cube_mesh.material.albedo[1],
        cube_mesh.material.albedo[2],
        cube_mesh.material.metallic,
        cube_mesh.material.roughness
    );
    println!(
        "  • Triangle: Albedo({:.1}, {:.1}, {:.1}), Metallic: {:.1}, Roughness: {:.1}",
        triangle_mesh.material.albedo[0],
        triangle_mesh.material.albedo[1],
        triangle_mesh.material.albedo[2],
        triangle_mesh.material.metallic,
        triangle_mesh.material.roughness
    );
    println!(
        "  • Point Cloud Mesh: Albedo({:.1}, {:.1}, {:.1}), Metallic: {:.1}, Roughness: {:.1}",
        point_cloud_mesh.material.albedo[0],
        point_cloud_mesh.material.albedo[1],
        point_cloud_mesh.material.albedo[2],
        point_cloud_mesh.material.metallic,
        point_cloud_mesh.material.roughness
    );
