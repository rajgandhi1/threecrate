...
use threecrate_io::{XyzCsvPoint, XyzCsvReader, XyzCsvWriteOptions, XyzCsvWriter};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, point) in points.iter().enumerate() {
        println!(
            "   Point {}: pos=({}, {}, {}), intensity={:?}, color={:?}, normal={:?}",
            i,
            point.position.x,
            point.position.y,
            point.position.z,
    ...
    XyzCsvWriter::write_detailed_points(
        &detailed_points,
        "output_detailed.csv",
        &complete_options,
    )?;
    ...
    println!(
        "   Auto-detected tab delimiter and read {} points",
        cloud.len()
    );
