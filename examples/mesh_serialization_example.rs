...
use threecrate_io::{
    AttributePreservingReader, AttributePreservingWriter, ExtendedTriangleMesh,
    MeshAttributeOptions, SerializationOptions,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Created pyramid with {} vertices and {} faces",
        vertices.len(),
        faces.len()
    );
    ...
    println!(
        "Initial mesh completeness: {:.1}%",
        extended_mesh.metadata.completeness_score * 100.0
    );
    ...
    println!(
        "   ✓ Normals computed: {}",
        extended_mesh.mesh.normals.is_some()
    );
    println!("   ✓ UVs generated: {}", extended_mesh.uvs.is_some());
    println!(
        "   ✓ Tangents computed: {}",
        extended_mesh.tangents.is_some()
    );
    println!(
        "   Completeness after processing: {:.1}%",
        extended_mesh.metadata.completeness_score * 100.0
    );
    ...
    // Write to OBJ
    AttributePreservingWriter::write_extended_mesh(
        &extended_mesh,
        obj_file,
    ...
    println!(
        "   OBJ mesh completeness: {:.1}%",
        obj_mesh.metadata.completeness_score * 100.0
    );

    // Compare meshes
    let obj_differences =
        threecrate_io::serialization::utils::compare_meshes(&extended_mesh, &obj_mesh)?;
    ...
    println!(
        "   PLY mesh completeness: {:.1}%",
        ply_mesh.metadata.completeness_score * 100.0
    );

    // Compare meshes
    let ply_differences =
        threecrate_io::serialization::utils::compare_meshes(&extended_mesh, &ply_mesh)?;
    ...
    let converted_mesh =
        AttributePreservingReader::read_extended_mesh(converted_file, &serialization_options)?;

    let conversion_differences =
        threecrate_io::serialization::utils::compare_meshes(&extended_mesh, &converted_mesh)?;
    if conversion_differences.is_empty() {
        println!("   ✓ Perfect cross-format conversion!");
    } else {
        println!(
            "   ⚠ Cross-format differences: {}",
            conversion_differences.len()
        );
    ...
    let validation_warnings =
        threecrate_io::mesh_attributes::utils::validate_round_trip(&extended_mesh)?;
    ...
    println!(
        "   Source format: {:?}",
        extended_mesh.metadata.source_format
    );
    println!(
        "   Normals computed: {}",
        extended_mesh.metadata.normals_computed
    );
    println!(
        "   Tangents computed: {}",
        extended_mesh.metadata.tangents_computed
    );
    ...
    println!(
        "   Missing attributes: {:?}",
        extended_mesh.metadata.missing_attributes()
    );
