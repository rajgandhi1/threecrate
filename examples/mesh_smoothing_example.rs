...
use threecrate_algorithms::{
    smooth_hc, smooth_laplacian, smooth_taubin, HcSmoothingConfig, LaplacianSmoothingConfig,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Spike vertex z before smoothing: {:.4}\n",
        mesh.vertices[12].z
    );
    ...
    println!(
        "   Spread: {:.4} (larger = less shrinkage than Laplacian)",
        avg_spread(&tau)
    );
