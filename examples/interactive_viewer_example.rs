...
mod viewer {
    ...
    fn create_torus_volume(
        ...
        let voxel = [
            size[0] / (res[0] - 1) as f32,
            size[1] / (res[1] - 1) as f32,
            size[2] / (res[2] - 1) as f32,
        ];
    ...
    pub fn update_scene(
        ...
        match state.mode {
            DisplayMode::Mesh(t) => {
                ...
                commands.spawn((
                    Mesh3d(meshes.add(build_mesh(t))),
                    MeshMaterial3d(materials.add(make_mesh_material(state.color_index))),
                    Transform::default(),
                    SceneMesh,
                ));
            }
