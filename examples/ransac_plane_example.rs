...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Inlier percentage: {:.1}%",
        (inliers.len() as f32 / simple_cloud.len() as f32) * 100.0
    );
    ...
    println!(
        "   Inlier percentage: {:.1}%",
        (inliers.len() as f32 / noisy_cloud.len() as f32) * 100.0
    );
    ...
    println!(
        "   Inlier percentage: {:.1}%",
        (inliers.len() as f32 / tilted_cloud.len() as f32) * 100.0
    );
    ...
    println!(
        "   Inlier percentage: {:.1}%",
        (inliers.len() as f32 / multi_plane_cloud.len() as f32) * 100.0
    );
    ...
    println!(
        "   Results identical: {}",
        coefficients == coefficients_alias && inliers == inliers_alias
    );
