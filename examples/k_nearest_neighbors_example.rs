...
use rand::Rng;
use threecrate_algorithms::nearest_neighbor::{BruteForceSearch, KdTree};
use threecrate_algorithms::point_cloud_ops::PointCloudNeighbors;
use threecrate_core::{NearestNeighborSearch, Point3f, PointCloud};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let results_match = kdtree_result.len() == brute_result.len()
        && kdtree_result
            .iter()
            .zip(brute_result.iter())
            .all(|(a, b)| a.0 == b.0 && (a.1 - b.1).abs() < 1e-6);
    ...
    println!(
        "   Speedup: {:.2}x",
        brute_time.as_nanos() as f64 / kdtree_time.as_nanos() as f64
    );
    println!();
    ...
    println!(
        "   radius = 0 result: {} neighbors",
        zero_radius_result.len()
    );

    // Negative radius
    let negative_radius_result = cloud.find_radius_neighbors(&query_point, -1.0);
    println!(
        "   negative radius result: {} neighbors",
        negative_radius_result.len()
    );
